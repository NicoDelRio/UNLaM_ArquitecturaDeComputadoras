...
*-------------------------------------------------------------------------------------------------------------------------------------------
; Programa: 02-Suma en N Bytes (Inicializando RESULTADO en 0 en el codigo)

; Declaracion de constantes 
RAM			EQU		$0000			; Idem a RAM0Start (Memory Configuration) Para el ORG de 1ra
ROM			EQU		$8000			; Idem a ROM0Start (Memory Configuration) Para el ORG de 2da Parte
BYTES			EQU		16			; EQU = Constante

; Declaracion de variables
; Etiqueta		; Instruccion	; Operando		; Comentario
			ORG		RAM			; Dir donde se declaran las variables
...
; Codigo del Programa
; Etiqueta		; Instruccion	; Operando		; Comentario
			ORG		ROM			; GUARDA A PARTIR DEL ROM0Start
								; PARA LIMPIAR RDO
			LDX		#(RESULTADO+BYTES)	; Guarda en X la dir de RESULTADO menos significativo. PUNTERO
			LDAB		#(BYTES+1)		; (Funcion FOR). Guarda en B la cantidad de iteracciones.
CLEAR_RDO		CLR		0,X			; CLR: Clear Memory Byte: (0 -> M)
			DEX					; IX-1 -> IX. Decrementa X
			DECB					; B-1 -> B. Decrementa B. Si quedo en CERO, Z=1
			BNE		CLEAR_RDO		; (?Z=0). Si Z=1, sale del bucle CLEAR_RDO.

			LDX		#(OPERANDO1+BYTES-1)	; Guarda en X la dir de OPERANDO1 menos significativo. PUNTERO
			LDAB		#(BYTES)		; (Funcion FOR). Guarda en B la cantidad de iteracciones.
...
SUMAS_PARC		LDAA		0,X			; Guarda en Acum A dato de la dir de REG X (ACTUAL)
			...
			DEX					; IX-1 -> IX. Decrementa X
			DECB					; B-1 -> B. Decrementa B. Si quedo en CERO, Z=1
			BNE		SUMAS_PARC		; (?Z=0). Si Z=1, sale del bucle SUMAS_PARC.
			BHS		NO_CARRY		; (Funcion IF). (?C=0). Si C=0 salta a NO_CARRY. Si C=1, byte mas sign de RESULTADO = 1.
...
; Etiqueta		; Instruccion	; Operando		; Comentario
			...
			DB		$A7,$A6,$10,$9B,$F5,$07,$44,$E6,$59,$42,$CF,$2A,$19,$51,$9A,$A9

; RESULTADO INICIALIZADO EN 0 EN EL CODIGO
; Solo seria necesario limpiar el byte mas significativo (por si estaba en 1, y no se produce carry),
; ya que el resto de los bytes los pisara si o si.
