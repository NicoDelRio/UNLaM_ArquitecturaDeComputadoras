; 12-Tipo_Examen_Vector_de_256_elementos_de(16bits)(con_ERROR)
...
; ERROR EN "cantN" POR SER LOS 256 ELEMENTOS DEL VECTOR NEGATIBOS (cantN = 00 Y deberia ser 01 00)
...
BUCLE			LDD		0,Y			; BUCLE SE REPITE "CICLOS" VECES
			BNE		NO_CERO			; (?Z=0). IF(!D==0) -> SALTA A NO_CERO
			INC		cantC			; IF(D==0) -> cantC++
NO_CERO			BPL		NO_NEG			; (?N=0). IF(!D<0) -> SALTA A NO_NEG
			INC		cantN			; IF(D<0) -> cantN++
NO_NEG			LDX		#MULTIPLO		; X = MULTIPLO
			IDIV					; D/X->X , r->D
			TSTA					; (A-0). SE ANALIZA "A" POR SI EL RESTO FUESE 256 (B SERIA $00).
			BNE		NO_MULTIPLO		; IF(!A==0) -> SALTA A NO_MULTIPLO
			TSTB					; (B-0). SI r=0 (ES MULTIPLO) -> Z=0
			BNE		NO_MULTIPLO		; IF(!B==0) -> SALTA A NO_MULTIPLO
			INC		cantO			; IF(A==0 && B==0) -> cantO++
